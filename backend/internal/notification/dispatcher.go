...
func (d *Dispatcher) Dispatch(ctx context.Context, eventType string, appointmentID uuid.UUID, user repository.User) {
	apptNullID := uuid.NullUUID{UUID: appointmentID, Valid: appointmentID != uuid.Nil}

	if user.NotifyEmail && user.Email != "" {
		go func() {
			logEntry, _ := d.queries.CreateNotificationLog(ctx, repository.CreateNotificationLogParams{
			...
			err := d.email.Send(ctx, eventType, user)
			if err != nil {
				log.Printf("email notification failed for user %s: %v", user.ID, err)
				d.queries.UpdateNotificationLogStatus(ctx, logEntry.ID, "failed",
					sql.NullString{String: err.Error(), Valid: true})
				return
			}
			d.queries.UpdateNotificationLogStatus(ctx, logEntry.ID, "sent", sql.NullString{})
		}()
	}
	...
	if user.NotifyWhatsapp && user.Phone.Valid && user.Phone.String != "" {
		go func() {
			...
			err := d.whatsapp.Send(ctx, eventType, user)
			if err != nil {
	...
	if user.NotifyPush && user.FCMToken.Valid && user.FCMToken.String != "" {
		go func() {
			...
			err := d.fcm.Send(ctx, eventType, user)
			if err != nil {
